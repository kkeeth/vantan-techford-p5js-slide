...
最後に `rotate()` とか，`translate()` の座標位置を変更していくとどんどん曲線が変化し，バラ曲線の違った面を味わえるので，楽しんでみてください💁
...
function setup() {
  ...
  vertex(-10, -8);
  vertex(-35, 0);
  // vertex(-10, 10); // このようにしっかり繋げてあげることが重要
  endShape();
}
```
