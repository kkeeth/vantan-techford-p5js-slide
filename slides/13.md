...
#### Web の場合，イベントはブラウザの画面の中で発火され，それぞれの操作に応じて利用可能な __イベントハンドラ__ や __イベントリスナ__ と呼ばれるコードが実行される

<br />


* `イベントリスナ(Event Listener)`: イベントの発生を監視し動作するコード
* `イベントハンドラ(Event Handler)`: 発生したイベントの応答として動作するコード
...
他にもいくつかマウス操作用の関数もありますし，同様に __キーボード操作用の関数__ もありますので，公式サイトを見てみてください💁
...
<img src="/assets/13/Rose.png" class="mx-auto" width="420" alt="バラ曲線の一覧画像" />
...
```js
 ...
 for (let theta = 0; theta < TAU; theta += 0.01) {
   ...
   const r0 = pow(-1, mod) * (A - floor(A)) + mod;
    ...
    // r(x):=r0(x) + 2 * h(r0(x));
   const r = r0 + 2 * calculateH(r0);

   let x = petalSize * r * cos(theta);
   let y = petalSize * r * sin(theta);
   vertex(x, y);
 }
 endShape(CLOSE);
```
